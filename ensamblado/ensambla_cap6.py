#!/usr/bin/env python3
"""Ensambla sitio/muestreo/capitulo-6-probabilidades-desiguales.html desde la
plantilla, los módulos escritos aparte y las cadenas de código ya ejecutadas.

Mismo mecanismo que ensambla_cap5.py. Este capítulo es NUEVO: cubre las
semanas 12-13, que el material viejo no tocaba.
"""
...
DESTINO = RAIZ / "sitio" / "muestreo" / "capitulo-6-probabilidades-desiguales.html"
...
def main():
    ...
    reemplazos = [
        ('content="Plantilla base para los capítulos del material de Muestreo Estadístico '
         ...
         'muestreo con probabilidades desiguales, estimador de Hansen-Thompson y de '
         'Horvitz-Thompson, diseños PPT y de Poisson, varianza de Sen-Yates-Grundy, diseños '
         'autoponderados y muestreo por importancia, con simuladores y código en R y Python."'),
        ('<meta name="keywords" content="muestreo estadístico, muestreo probabilístico, '
         ...
         'R, Python, UnBosque">',
         '<meta name="keywords" content="probabilidades desiguales, PPT, PPS, Hansen-Hurwitz, '
         'Horvitz-Thompson, Sen-Yates-Grundy, muestreo de Poisson, metodo de Lahiri, metodo '
         'acumulativo, inclusion forzosa, diseno autoponderado, muestreo por importancia, active '
         'learning, survey, sampling, R, Python, Lohr, Gutiérrez, UnBosque">'),
        ('<title>Plantilla de capítulo — Muestreo Estadístico</title>',
         '<title>Capítulo 6 · Probabilidades desiguales — Muestreo Estadístico</title>'),
        ('<p class="text-xs text-white/70 font-medium tracking-wide" style="margin:0; text-align:left;">PLANTILLA BASE •\n'
         ...
         '<p class="text-xs text-white/70 font-medium tracking-wide" style="margin:0; text-align:left;">CAPÍTULO 6 •\n'
         '              PROBABILIDADES DESIGUALES • SEMANAS 12–13 • UNBOSQUE</p>'),
        ('<p class="text-xs mt-1 text-white/60" style="text-align:center;">Plantilla de capítulo • UnBosque 2026\n        </p>',
         '<p class="text-xs mt-1 text-white/60" style="text-align:center;">Muestreo Estadístico • Capítulo 6 • UnBosque 2026-II\n        </p>'),
        ('<i class="fas fa-layer-group text-xl text-white" aria-hidden="true"></i>',
         '<i class="fas fa-scale-unbalanced text-xl text-white" aria-hidden="true"></i>'),
    ]
    ...
    # ---------------------------------------------------------------- módulos
    modulos = "".join((RAIZ / "ensamblado" / "modulos" / "cap6" / f).read_text(encoding="utf-8")
    ...
    # ---------------------------------------------------------------- courseData + datos
    datos = json.loads((RAIZ / "precalculo" / "salidas" / "cap6_datos.json").read_text(encoding="utf-8"))
    course = """    const courseData = {
      title: "Probabilidades desiguales",
      modules: [
        { id: 1, title: "¿Por qué probabilidades desiguales?", shortTitle: "Por qué", duration: "20 min" },
        { id: 2, title: "El estimador de Hansen–Hurwitz", shortTitle: "Hansen–Hurwitz", duration: "22 min" },
        { id: 3, title: "Cómo se sortea: acumulativo y Lahiri", shortTitle: "Selección", duration: "20 min" },
        { id: 4, title: "Sin reemplazo: Horvitz–Thompson otra vez", shortTitle: "HT sin reemplazo", duration: "22 min" },
        { id: 5, title: "El diseño πPT y cuándo gana", shortTitle: "πPT", duration: "25 min" },
        { id: 6, title: "El diseño de Poisson", shortTitle: "Poisson", duration: "20 min" },
        { id: 7, title: "Estimar la varianza: SYG", shortTitle: "Varianza SYG", duration: "25 min" },
        { id: 8, title: "Cuándo el HT se comporta mal", shortTitle: "El elefante", duration: "18 min" },
        { id: 9, title: "PPT en dos etapas", shortTitle: "Dos etapas", duration: "22 min" },
        { id: 10, title: "Importancia y active learning", shortTitle: "Importancia", duration: "20 min" },
        { id: 11, title: "Autoevaluación y ejercicios guiados", shortTitle: "Autoevaluación", duration: "35 min" }
      ]
    };

    // ================================================================
    // Datos del capítulo, generados por precalculo/genera_cap6.R con
...
""" % (datos["meta"]["semilla"], json.dumps(datos, ensure_ascii=False, separators=(",", ":")))
    ...
    # ---------------------------------------------------------------- simuladores + quiz
    sims = (RAIZ / "ensamblado" / "modulos" / "cap6" / "simuladores.js").read_text(encoding="utf-8")
    antes, despues = corta(
        html,
    ...
    # ---------------------------------------------------------------- código
    codigo = {}
    codigo.update(bloques_de(RAIZ / "ensamblado" / "codigo" / "cap6" / "cadena.R"))
    codigo.update(bloques_de(RAIZ / "ensamblado" / "codigo" / "cap6" / "cadena.py"))
    faltan = []
