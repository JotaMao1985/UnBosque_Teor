    ...
    GLOSARIOS['norespuesta'] = {
      ...
      nota: 'Lohr escribe la no respuesta en términos de <em>respondientes</em> y <em>no respondientes</em>: ' +
        'el conjunto de respondientes es una segunda muestra, con su propio diseño desconocido, dentro de ' +
        'la muestra que sí se eligió. Los guiones de la columna de Gutiérrez no son un olvido: su libro ' +
        'no desarrolla la no respuesta ni la imputación con notación propia —las menciona al pasar en el ' +
        'capítulo de muestreo en dos fases—, y sus símbolos $\\delta_k$ y $q_k$ significan aquí otra cosa ' +
        '(las variables dummy de post-estrato y las ponderaciones de la distancia de calibración). ' +
        'Las reglas de Rubin tampoco aparecen con fórmula en ninguno de los dos libros: la notación de ' +
        'esas tres filas es la de este material.',
      filas: [
        { concepto: 'Indicador de respuesta', aqui: 'R_k', lohr: 'R_i', gutierrez: '—', r: 'is.na(y)' },
        { concepto: 'Propensión a responder', aqui: '\\phi_k', lohr: '\\phi_i', gutierrez: '—',
          r: 'glm(R ~ x, family = binomial)' },
        { concepto: 'Tasa de respuesta', aqui: 'R', lohr: 'N_R/N', gutierrez: '—', r: 'mean(R)' },
        { concepto: 'Media de los respondientes', aqui: '\\bar y_R', lohr: '\\bar y_R', gutierrez: '\\bar y_r',
          r: 'mean(y, na.rm = TRUE)' },
        { concepto: 'Clase de ajuste', aqui: 'c', lohr: '\\text{weighting class}', gutierrez: 'U_g',
          r: 'cut(x, cortes)' },
        ...
        { concepto: 'Valor imputado', aqui: '\\tilde y_k', lohr: '—', gutierrez: '—',
          r: 'y[falta] <- ...' },
        { concepto: 'Varianza entre imputaciones', aqui: 'B', lohr: '—', gutierrez: '—',
          r: 'var(sapply(imps, mean))' },
        { concepto: 'Varianza total de Rubin', aqui: 'T = \\bar U + (1+1/m)B', lohr: '—',
          gutierrez: '—', r: 'vcov(MIcombine(...))' }
      ]
    };
    ...
    AUTOEVALUACIONES['cap8'] = [
      ...
      },
      {
        ...
        retroAcierto: 'Todas menos la de MCAR. Esa es falsa: bajo MCAR la media de los respondientes ya es insesgada y el ajuste no aporta nada — solo desiguala los pesos y añade varianza. Ajustar «por si acaso» tiene precio.',
        retroFallo: 'Son las tres que no hablan de MCAR. La de MCAR describe justo lo contrario: es el único caso en que no hay nada que corregir, y ajustar solo cuesta varianza.'
      },
      {
        tipo: 'numerica',
        modulo: 2,
      ...
      },
      {
        ...
        retroAcierto: 'Todas menos la de MNAR. Esa es falsa por la misma razón que en el ajuste de pesos: la imputación se apoya en un modelo con las variables observadas, y si lo que falta depende del valor que falta, ningún modelo construido sobre lo observado lo ve.',
        retroFallo: 'Son las tres que describen una propiedad de cada técnica. La de MNAR repite el error del módulo 3 en el terreno de la imputación: MNAR no se arregla con técnica, se arregla con datos de los ausentes.'
      },
      {
        tipo: 'grafico',
        modulo: 6,
        descripcionGrafico: 'Tres barras con la cobertura empírica del intervalo nominal del 95 %, sobre un eje que va de 0,80 a 1, y una línea horizontal en 0,95 rotulada «nominal 95 %». «Datos completos» llega a 0,9265 y es la más alta de las tres, aunque se queda por debajo de la línea; «imputación única» es la más corta, 0,8715; «imputación múltiple» sube a 0,9115, entre las otras dos y todavía por debajo de la línea.',
